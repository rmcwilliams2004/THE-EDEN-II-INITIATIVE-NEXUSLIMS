...
import { Sprout, Award, CheckCircle2, ShieldCheck, Download, Share2, Layers, Binary, Copy, Check, Sparkles, ExternalLink, Globe, ArrowRight, QrCode } from 'lucide-react';
import { RetiredCreditsBarChart } from './RetiredCreditsBarChart';
import { HederaQrCodeVerification } from './HederaQrCodeVerification';
    ...
    <div className="space-y-6">
      <div className="bg-slate-900/80 p-4 rounded-2xl border border-slate-800 flex flex-col sm:flex-row sm:items-center justify-between gap-3">
        ...
        <button
          ...
          className="px-3.5 py-2 rounded-xl bg-emerald-500/20 hover:bg-emerald-500/30 text-emerald-400 border border-emerald-500/40 text-xs font-mono font-bold flex items-center gap-1.5 transition-all self-start sm:self-auto shrink-0 cursor-pointer"
        >
          <Globe className="w-3.5 h-3.5" />
          <span>Live Carbon & Regulatory Intel</span>
      ...
      <div className="bg-slate-900/60 p-6 rounded-2xl border border-slate-800 space-y-4">
        ...
        {/* Animated Retirement Certificate with fade-in and smooth slide-up effect */}
        {certificateGenerated && certificateData && (
          <div
            key={certificateData.id}
            className={`mt-4 p-6 bg-slate-950/90 border-2 border-purple-500/50 rounded-2xl space-y-4 font-mono text-xs shadow-2xl relative overflow-hidden backdrop-blur-sm ${
              certificateGenerated ? 'animate-fade-slide-up' : ''
            }`}
          >
            ...
            <div className="flex flex-col sm:flex-row sm:items-center justify-between text-purple-400 font-bold border-b border-slate-800/80 pb-3 gap-2">
              ...
              <div className="flex items-center gap-2 text-[11px] text-slate-400">
                <a
                  ...
                  className="px-2.5 py-1 bg-purple-950/80 hover:bg-purple-900 border border-purple-500/50 hover:border-purple-400 text-purple-200 hover:text-white rounded-lg flex items-center gap-1.5 transition-all shadow-sm font-mono text-[11px] font-bold cursor-pointer"
                >
                ...
                <button
                  ...
                  className="p-1.5 hover:bg-slate-800 rounded-lg border border-slate-800 text-slate-400 hover:text-white transition-colors cursor-pointer"
                >
            ...
            {/* Hedera Physical Auditor QR Verification Module */}
            <HederaQrCodeVerification
              txHash={certificateData.txHash}
              certificateId={certificateData.id}
              amountKg={certificateData.amount}
              topicId="0.0.984210"
              beneficiary="SunValley Ag Enterprises"
              timestamp={certificateData.timestamp}
            />
